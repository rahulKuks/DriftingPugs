...
//	public Ocean ocean;
	...
	public float randomRotationFactor = 0.3f;
	...
	public static Vector3 PerlinVector3 (float time) {
		...
		for (int i = 0; i < 3; i++) {
			rotationVector [i] = (Mathf.PerlinNoise (time , i * 100) - 0.5f) * 2.0f; 
			// Sets the x, y and z to different factors based off a perlin noise function, (the x inputed into the function is always t but the y inputed changes).
		}
	...
	{
		//return ocean == null ? 79.0f : ocean.GetWaterHeightAtLocation(x, z);
		return 79.5f;
	}
	...
	private void FixedUpdate()
	{
		...
		foreach (var point in voxels)
		{
			...
			if (wp.y - voxelHalfHeight < waterLevel)
			{
				...
				GetComponent<Rigidbody>().transform.Rotate ((PerlinVector3 (Time.time) - PerlinVector3 (Time.time - Time.deltaTime)) * randomRotationFactor);
