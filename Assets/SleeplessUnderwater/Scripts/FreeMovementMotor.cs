...
public class FreeMovementMotor : MonoBehaviour
{
	...
	void FixedUpdate () {
        ...
        if (transform.gameObject.GetComponent<PlayerMoveController>().m_IsSwimming == true)
        {
        ...
        if (Input.GetKey(KeyCode.Space) && transform.gameObject.GetComponent<PlayerMoveController>().m_IsSwimming == true)
        {
            GetComponent<Rigidbody>().AddForce(Vector3.up * 80, ForceMode.Acceleration);
        }
        // Crouching to swim down
        if (Input.GetKey(KeyCode.C) && transform.gameObject.GetComponent<PlayerMoveController>().m_IsSwimming == true)
        {
            GetComponent<Rigidbody>().AddForce(-Vector3.up * 80, ForceMode.Acceleration);
        }
    }
