...
public class SoundController : MonoBehaviour
{
	...
	[SerializeField] private AudioSource lake;
	...
	[SerializeField] private float lakeSpaceCrossfadeDuration = 1.0f;
    ...
    {
		StartCoroutine(Crossfade(lake, space, lakeSpaceCrossfadeDuration));
    }
	...
	public void EndResolution(float fadeDuration)
	{
		StartCoroutine(FadeOut(resolution, fadeDuration));
	}
	...
	private IEnumerator FadeOut(AudioSource source, float duration)
	{
		...
		while (progress <= duration)
		{
			...
			source.volume = Mathf.Min(1 - (progress / duration), 1.0f);
