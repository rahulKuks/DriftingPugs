...
namespace TeeBee
{
	...
	int InternalRegisterEffectDefinition(UnityAudioEffectDefinition& definition)
	{
		...
		RegisterParameter(definition, "Seed", "", 0.0f, 1000.0f, 0.0f, 1.0f, 1.0f, P_SEED);
		RegisterParameter(definition, "LowNote", "semitones", 0.0f, 100.0f, 24.0f, 1.0f, 1.0f, P_MINNOTE);
		RegisterParameter(definition, "HighNote", "semitones", 0.0f, 100.0f, 48.0f, 1.0f, 1.0f, P_MAXNOTE);
		RegisterParameter(definition, "Cutoff", "Hz", 0.0f, kMaxSampleRate, 1000.0f, 1.0f, 3.0f, P_CUT);
		RegisterParameter(definition, "Envelope", "Hz", 0.0f, kMaxSampleRate, 500.0f, 1.0f, 3.0f, P_ENV);
		RegisterParameter(definition, "CutRnd", "Hz", 0.0f, kMaxSampleRate, 0.0f, 1.0f, 3.0f, P_CUTRND);
		RegisterParameter(definition, "EnvRnd", "Hz", 0.0f, kMaxSampleRate, 0.0f, 1.0f, 3.0f, P_ENVRND);
		RegisterParameter(definition, "Decay", "s", 0.0f, 5.0f, 0.5f, 1.0f, 3.0f, P_DECAY);
		RegisterParameter(definition, "Resonance", "%", 0.0f, 1.0f, 0.2f, 100.0f, 2.0f, P_RES);
		RegisterParameter(definition, "Distortion", "%", 0.0f, 100.0f, 3.0f, 100.0f, 1.0f, P_DIST);
		RegisterParameter(definition, "Glide", "s", 0.001f, 1.0f, 0.01f, 1.0f, 3.0f, P_GLIDE);
		RegisterParameter(definition, "BPM", "BPM", 10.0f, 300.0f, 120.0f, 1.0f, 1.0f, P_BPM);
		RegisterParameter(definition, "LFOFreq", "Hz", 0.0f, 50.0f, 0.1f, 1.0f, 3.0f, P_LFOFREQ);
		RegisterParameter(definition, "LFOCut", "Hz", 0.0f, kMaxSampleRate, 0.0f, 1.0f, 3.0f, P_LFOCUT);
		RegisterParameter(definition, "LFOCutEnv", "Hz", 0.0f, kMaxSampleRate, 0.0f, 1.0f, 3.0f, P_LFOCUTENV);
	...
	UNITY_AUDIODSP_RESULT UNITY_AUDIODSP_CALLBACK ProcessCallback(UnityAudioEffectState* state, float* inbuffer, float* outbuffer, unsigned int length, int inchannels, int outchannels)
	{
		...
		for(unsigned int n = 0; n < length; n++)
		{
			...
			if(cut < 0.0f) cut = 0.0f; else if(cut > max_cut) cut = max_cut;
