...
namespace Multiband
{
	...
	static char* tmpstr(const char* fmtstr, ...)
	{
		static char buf[512];
		va_list args;
	...
	int InternalRegisterEffectDefinition(UnityAudioEffectDefinition& definition)
	{
		...
		RegisterParameter(definition, "HighFreq", "Hz", 0.01f, 24000.0f, 5000.0f, 1.0f, 3.0f, P_HighFreq);
		for(int i = 0; i < 3; i++)
			RegisterParameter(definition, tmpstr("%sGain", bandname[i]), "dB", -100.0f, 100.0f, 0.0f, 1.0f, 1.0f, P_LowGain + i);
		for(int i = 0; i < 3; i++)
			RegisterParameter(definition, tmpstr("%sAttackTime", bandname[i]), "ms", 0.0f, 10.0f, 0.1f, 1000.0f, 4.0f, P_LowAttack + i);
		for(int i = 0; i < 3; i++)
			RegisterParameter(definition, tmpstr("%sReleaseTime", bandname[i]), "ms", 0.0f, 10.0f, 0.5f, 1000.0f, 4.0f, P_LowRelease + i);
		for(int i = 0; i < 3; i++)
			RegisterParameter(definition, tmpstr("%sThreshold", bandname[i]), "dB", -50.0f, 0.0f, -10.0f, 1.0f, 1.0f, P_LowThreshold + i);
		for(int i = 0; i < 3; i++)
			RegisterParameter(definition, tmpstr("%sRatio", bandname[i]), "%", 1.0f, 30.0f, 1.0f, 100.0f, 1.0f, P_LowRatio + i);
		for(int i = 0; i < 3; i++)
			RegisterParameter(definition, tmpstr("%sKnee", bandname[i]), "dB", 0.0f, 40.0f, 10.0f, 1.0f, 1.0f, P_LowKnee + i);
		RegisterParameter(definition, "FilterOrder", "", 1.0f, (float)MAXORDER, 1.0f, 1.0f, 1.0f, P_FilterOrder);
	...
	int UNITY_AUDIODSP_CALLBACK GetFloatBufferCallback (UnityAudioEffectState* state, const char* name, float* buffer, int numsamples)
	{
		...
		{
			data->bandsplit[0][0][0].StoreCoeffs(buffer);
			data->bandsplit[0][0][1].StoreCoeffs(buffer);
			data->bandsplit[0][0][2].StoreCoeffs(buffer);
			data->bandsplit[0][0][3].StoreCoeffs(buffer);
		}
		else
#endif
	...
	UNITY_AUDIODSP_RESULT UNITY_AUDIODSP_CALLBACK ProcessCallback(UnityAudioEffectState* state, float* inbuffer, float* outbuffer, unsigned int length, int inchannels, int outchannels)
	{
		...
		for(int i = 0; i < inchannels; i++)
		{
			...
			for(int k = 0; k < MAXORDER; k++)
			{
				...
				data->bandsplit[i][k][0].SetupLowpass(data->p[P_LowFreq], sr, qfactor);
				data->bandsplit[i][k][1].SetupHighpass(data->p[P_LowFreq], sr, qfactor);
				data->bandsplit[i][k][2].SetupLowpass(data->p[P_HighFreq], sr, qfactor);
				data->bandsplit[i][k][3].SetupHighpass(data->p[P_HighFreq], sr, qfactor);
			}
		}
