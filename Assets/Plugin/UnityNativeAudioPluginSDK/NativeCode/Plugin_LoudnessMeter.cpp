...
namespace LoudnessMeter
{
	...
	public:
		...
		void ReadBuffer(float* buffer, int numsamplesTarget, float windowLength, float samplerate, bool rms)
		{
			...
			RingBuffer& buf = (rms) ? rmsbuf : peakbuf;
	...
	int InternalRegisterEffectDefinition(UnityAudioEffectDefinition& definition)
	{
		...
		RegisterParameter(definition, "Active", "", 0.0f, 1.0f, 0.0f, 1.0f, 1.0f, P_Active);
		RegisterParameter(definition, "Window", "s", 0.1f, kMaxWindowLength, 1.0f, 1.0f, 1.0f, P_Window);
		RegisterParameter(definition, "YOffset", "dB", -200.0f, 200.0f, 0.0f, 1.0f, 1.0f, P_YOffset);
		RegisterParameter(definition, "YScale", "%", 0.001f, 10.0f, 1.0f, 100.0f, 1.0f, P_YScale);
		return numparams;
	}
