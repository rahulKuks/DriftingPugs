...
namespace TeeDee
{
	...
	int InternalRegisterEffectDefinition(UnityAudioEffectDefinition& definition)
	{
		...
		RegisterParameter(definition, "Seed", "", 0.0f, 100.0f, 0.0f, 1.0f, 1.0f, P_SEED);
		RegisterParameter(definition, "BPM", "BPM", 10.0f, 300.0f, 120.0f, 1.0f, 1.0f, P_BPM);
		RegisterParameter(definition, "Sine", "%", 0.0f, 1.0f, 1.0f, 100.0f, 1.0f, P_SINE);
		RegisterParameter(definition, "Noise", "%", 0.0f, 1.0f, 0.0f, 100.0f, 1.0f, P_NOISE);
		RegisterParameter(definition, "Cutoff", "Hz", 0.0f, kMaxSampleRate, 1000.0f, 1.0f, 3.0f, P_CUT);
		RegisterParameter(definition, "Resonance", "%", 0.0f, 1.0f, 0.2f, 100.0f, 3.0f, P_RES);
		RegisterParameter(definition, "Freq", "Hz", 0.0f, 1000.0f, 200.0f, 1.0f, 3.0f, P_FREQ);
		RegisterParameter(definition, "AmpDecay", "s", 0.0f, 5.0f, 0.5f, 1.0f, 3.0f, P_ADECAY);
		RegisterParameter(definition, "FilterDecay", "s", 0.0f, 5.0f, 0.5f, 1.0f, 3.0f, P_FDECAY);
		RegisterParameter(definition, "PitchDecay", "s", 0.0f, 5.0f, 0.5f, 1.0f, 3.0f, P_PDECAY);
		RegisterParameter(definition, "FilterType", "%", 0.0f, 1.0f, 0.0f, 100.0f, 1.0f, P_FILTERTYPE); // TODO: labels
		RegisterParameter(definition, "Distortion", "%", 0.0f, 100.0f, 3.0f, 100.0f, 1.0f, P_POSTDIST);
		RegisterParameter(definition, "AmpEnv", "%", 0.0f, 1.0f, 1.0f, 100.0f, 1.0f, P_AENV);
		RegisterParameter(definition, "FilterEnv", "%", 0.0f, kMaxSampleRate, 5000.0f, 1.0f, 3.0f, P_FENV);
		RegisterParameter(definition, "PitchEnv", "%", 0.0f, 1000.0f, 0.0f, 100.0f, 1.0f, P_PENV);
		RegisterParameter(definition, "PreDist", "%", 0.0f, 100.0f, 1.0f, 100.0f, 1.0f, P_PREDIST);
		RegisterParameter(definition, "InputMix", "%", 0.0f, 100.0f, 100.0f, 1.0f, 1.0f, P_INPUTMIX);
		RegisterParameter(definition, "NumSteps", "", 1.0f, 64.0f, 16.0f, 1.0f, 1.0f, P_NUMSTEPS);
		return numparams;
	}
	...
	UNITY_AUDIODSP_RESULT UNITY_AUDIODSP_CALLBACK ProcessCallback(UnityAudioEffectState* state, float* inbuffer, float* outbuffer, unsigned int length, int inchannels, int outchannels)
	{
		...
		float wetTarget = ((state->flags & UnityAudioEffectStateFlags_IsPlaying) && !(state->flags & UnityAudioEffectStateFlags_IsMuted)) ? 1.0f : 0.0f;
		...
		for(unsigned int n = 0; n < length; n++)
		{
			...
			if(cut < 0.0f) cut = 0.0f; else if(cut > sr_half) cut = sr_half;
