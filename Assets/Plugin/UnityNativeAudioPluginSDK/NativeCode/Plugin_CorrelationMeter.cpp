...
namespace CorrelationMeter
{
	...
	int InternalRegisterEffectDefinition(UnityAudioEffectDefinition& definition)
	{
		...
		RegisterParameter(definition, "Window", "s", 0.1f, 2.0f, 0.15f, 1.0f, 1.0f, P_Window);
		RegisterParameter(definition, "Scale", "%", 0.01f, 10.0f, 1.0f, 100.0f, 1.0f, P_Scale);
		return numparams;
	}
	...
	{
		EffectData* data = state->GetEffectData<EffectData>();
		RingBuffer& l = data->history[0];
		RingBuffer& r = data->history[1];
		int w1 = l.writeindex;
		int w2 = r.writeindex;
